...
if($_SERVER["REQUEST_METHOD"] =="POST"){
    ...
    compteur=".$compteur." WHERE id=".$id." AND theme='".$theme."';";
    $res = mysqli_query($conn, $sql);
    if (!$res){
    header("Location: cardRep.php?error=sqlerror");
    exit();
...
//Intitialisation premiere carte

$sql = "SELECT * FROM jeuActive WHERE id=".$id." AND theme='".$theme."';";
$res = mysqli_query($conn, $sql);
if (!$res){
    header("Location: cardRep.php?error=sqlerrora");
    exit();
}
if(mysqli_num_rows($res)==0){
    ...
    $res = mysqli_query($conn, $sql);
    if (!$res){
        header("Location: cardRep.php?error=sqlerrory");
        exit();
    }
    $tab = mysqli_fetch_assoc($res);
    $taille = $tab["COUNT(*)"];
    ...
    $sql = "INSERT INTO jeuActive(id, ordreActuel, ordreSuivant, theme, nbrErreur, currentScore)  VALUES (".$id.",'".$ordreActuel."','".$ordreSuivant."','".$theme."',0,0,1);";
    $res = mysqli_query($conn, $sql);
    if (!$res){
    header("Location: cardRep.php?error=sqlerror");
    exit();
